...
def main() -> None:
    ...
    """
    st.header("📚 Your Local PDF Assistant", divider="rainbow", anchor=False)
    ...
    with col1:
        # Model selection
        if available_models:
            selected_model = st.selectbox(":green[**__Pick a model available locally on your system ↓__**]", available_models,key="model_select")

        # Add checkbox for sample PDF
        use_sample = st.toggle("Use sample PDF (Scammer Agent Paper)", key="sample_checkbox")
        ...
        if use_sample:
            ...
            file_upload = st.file_uploader(":green[**Upload a PDF file ↓**]", type="pdf", accept_multiple_files=False,
                key="pdf_uploader")
    ...
    # Chat interface
    with col2:
        st.header("💬 Ask Questions", divider= "violet")
        message_container = st.container(height=450, border=True)
